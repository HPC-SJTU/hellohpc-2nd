...
int main(int argc, char *argv[])
{
      ...
      int checkrun = 0;
      char checkfilename[50] = "checkpoint.chk";
      int ID_type = 0;
      int Steps = 0;
      double StartTime = 0.0, TotalTime = 0.0;
      double AnasTime = 1.0e30, DumpTime = 1.0e30, d2DumpTime = 1.0e30, CheckTime = 1.0e30;
      double Courant = 0.5;
      double numepss = 0.0, numepsb = 0.0, numepsh = 0.0;
      int Symmetry = 1;
      int a_lev = 0, maxl = 2, decn = 1;
      double maxrex = 1.0, drex = 1.0;
      int surface_only = 0;
      int benchmark_repeats = 1;
      int benchmark_warmup = 0;
      int benchmark_mass = 0;
      int benchmark_case_seed = 0;
      double benchmark_radius_step = 0.0;
      string benchmark_result_file;
      int interpolation_order = 2 * ghost_width;
      // read parameter from file
      {
            ...
            ifstream inf(parameter_file.c_str(), ifstream::in);
            if (!inf.good() && myrank == 0)
            {
                  cout << "Can not open parameter file " << parameter_file << endl;
            ...
            for (int i = 1; inf.good(); i++)
            {
                  ...
                  if (status == -1)
                  {
                        cout << "error reading parameter file " << parameter_file << " in line " << i << endl;
                        MPI_Abort(MPI_COMM_WORLD, 1);
                  }
                  else if (status == 0)
                        continue;
                  ...
                  else if (sgrp == "Benchmark")
                  {
                        if (skey == "surface only")
                              surface_only = atoi(sval.c_str());
                        else if (skey == "repeats")
                              benchmark_repeats = atoi(sval.c_str());
                        else if (skey == "warmup")
                              benchmark_warmup = atoi(sval.c_str());
                        else if (skey == "include mass")
                              benchmark_mass = atoi(sval.c_str());
                        else if (skey == "case seed")
                              benchmark_case_seed = atoi(sval.c_str());
                        else if (skey == "radius step")
                              benchmark_radius_step = atof(sval.c_str());
                        else if (skey == "result file")
                              benchmark_result_file = sval;
                  }
                  else if (sgrp == "SurfaceIntegral" && skey == "interpolation order")
                        interpolation_order = atoi(sval.c_str());
            }
            inf.close();

            if (interpolation_order < 2 || interpolation_order > 2 * ghost_width || interpolation_order % 2 != 0)
            {
                  if (myrank == 0)
                        cout << "SurfaceIntegral::interpolation order must be an even value in [2, "
                             << 2 * ghost_width << "]" << endl;
                  MPI_Abort(MPI_COMM_WORLD, 1);
            }
            parameters::int_par.insert(map<string, int>::value_type("surface interpolation order", interpolation_order));
      ...
      if (myrank == 0)
      {
            ...
            // echo the micro definition in "macrodef.h"
      ...
      if (surface_only)
      {
            ...
            {
                  cout << endl;
                  cout << " Running dynamic-field surface integral benchmark " << endl;
                  cout << " BSSN evolution is disabled in this mode " << endl;
                  cout << endl;
            }
            ADM->Surface_Only_Benchmark(benchmark_repeats, benchmark_warmup,
                                        benchmark_mass != 0, benchmark_case_seed,
                                        benchmark_radius_step,
                                        benchmark_result_file.c_str());
      }
      else
      {
      // set up initial data
