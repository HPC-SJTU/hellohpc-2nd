...
static unsigned long long benchmark_mix(unsigned long long value)
{
  value ^= value >> 30;
...
static double benchmark_field_noise(int field_epoch, int case_seed,
                                    int grid_x, int grid_y, int grid_z,
                                    unsigned long long salt)
{
  unsigned long long value = static_cast<unsigned long long>(field_epoch + 1);
  value = benchmark_mix(value ^ (static_cast<unsigned long long>(case_seed + 1) << 32));
  value = benchmark_mix(value ^ static_cast<unsigned long long>(grid_x + 1));
  value = benchmark_mix(value ^ (static_cast<unsigned long long>(grid_y + 1) << 21));
  value = benchmark_mix(value ^ (static_cast<unsigned long long>(grid_z + 1) << 42));
  value = benchmark_mix(value ^ salt);
  const int sample = static_cast<int>(value & 0xfffffULL);
  return (sample - 524287.5) / 524288.0;
}
...
void bssn_class::Initialize()
{
  ...
  GH = new cgh(0, ngfs, Symmetry, parameter_file.c_str(), checkrun, ErrorMonitor);
  if (checkrun)
    CheckPoint->readcheck_cgh(PhysTime, GH, myrank, nprocs, Symmetry);
  else
    GH->compose_cgh(nprocs);
#ifdef WithShell
  SH = new ShellPatch(0, ngfs, parameter_file.c_str(), Symmetry, myrank, ErrorMonitor);
...
void bssn_class::Surface_Only_Benchmark(int repeats, int warmup, bool include_mass,
                                       int case_seed, double radius_step,
                                       const char *result_file)
{
  if (a_lev < 0 || a_lev >= GH->levels)
  {
    if (myrank == 0)
      cout << "Surface benchmark: invalid analysis level " << a_lev << endl;
    MPI_Abort(MPI_COMM_WORLD, 1);
  }

  repeats = Mymax(1, repeats);
  warmup = Mymax(0, warmup);
  if (case_seed < 0 || radius_step < 0)
  {
    if (myrank == 0)
      cout << "Surface benchmark: invalid benchmark parameters" << endl;
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  if (maxl < 2 || decn < 1 || maxrex <= 0 || maxrex - (decn - 1) * drex <= 0)
  {
    if (myrank == 0)
      cout << "Surface benchmark: invalid mode or detector configuration" << endl;
    MPI_Abort(MPI_COMM_WORLD, 1);
  }

  double scale = Mymax(1.0, maxrex);
  double real_scale = 1.0 + 0.0025 * (case_seed % 29);
  double imag_scale = 0.025 + 0.0007 * (case_seed % 31);
  double linear_x = 0.0008 * (1 + case_seed % 11);
  double linear_y = 0.0007 * (1 + case_seed % 13);
  double linear_z = 0.0006 * (1 + case_seed % 17);
  double mixed_xy = 0.0004 * (1 + case_seed % 7);
  double mixed_yz = 0.0003 * (1 + case_seed % 5);
  double mixed_xz = 0.0002 * (1 + case_seed % 3);
  ...
  double Rout[7] = {0, 0, 0, 0, 0, 0, 0};
  double Rout_accum[7] = {0, 0, 0, 0, 0, 0, 0};
  double wave_r = 0;
  double wave_i = 0;
  double spectral_energy = 0;
  double local_time = 0;

  MyList<Patch> *Pp = GH->PatL[a_lev];
  while (Pp)
  {
      ...
      if (myrank == cg->rank)
      {
        for (int k = 0; k < cg->shape[2]; k++)
          for (int j = 0; j < cg->shape[1]; j++)
            ...
            {
              int n = i + j * cg->shape[0] + k * cg->shape[0] * cg->shape[1];
              double scaled_x = cg->X[0][i] / scale;
              double scaled_y = cg->X[1][j] / scale;
              double scaled_z = cg->X[2][k] / scale;
              double scaled_xy = scaled_x * scaled_y;
              double scaled_yz = scaled_y * scaled_z;
              double scaled_xz = scaled_x * scaled_z;

              cg->fgfs[phi0->sgfn][n] = 0.02;
              cg->fgfs[trK0->sgfn][n] = 0.001;
              cg->fgfs[gxx0->sgfn][n] = 0.010;
              cg->fgfs[gxy0->sgfn][n] = 0.004 * scaled_xy;
              cg->fgfs[gxz0->sgfn][n] = 0.004 * scaled_xz;
              cg->fgfs[gyy0->sgfn][n] = -0.006;
              cg->fgfs[gyz0->sgfn][n] = 0.004 * scaled_yz;
              cg->fgfs[gzz0->sgfn][n] = 0.003;
              cg->fgfs[Axx0->sgfn][n] = 0.002;
              cg->fgfs[Axy0->sgfn][n] = 0.002 * scaled_xy;
              cg->fgfs[Axz0->sgfn][n] = 0.002 * scaled_xz;
              cg->fgfs[Ayy0->sgfn][n] = -0.001;
              cg->fgfs[Ayz0->sgfn][n] = 0.002 * scaled_yz;
              cg->fgfs[Azz0->sgfn][n] = -0.001;
              cg->fgfs[Gmx0->sgfn][n] = 0.002 * scaled_x;
              cg->fgfs[Gmy0->sgfn][n] = 0.002 * scaled_y;
              cg->fgfs[Gmz0->sgfn][n] = 0.002 * scaled_z;
            }
      }
      if (BP == Pp->data->ble)
        break;
  ...
  for (int benchmark_iteration = -warmup; benchmark_iteration < repeats;
       benchmark_iteration++)
  {
    const int field_epoch = benchmark_iteration + warmup + 1;
    const double epoch_x = 0.00035 * (1 + (field_epoch * 17 + case_seed) % 23);
    const double epoch_y = 0.00031 * (1 + (field_epoch * 29 + case_seed) % 19);
    const double epoch_z = 0.00027 * (1 + (field_epoch * 37 + case_seed) % 17);
    const double epoch_xy = 0.00019 * (1 + (field_epoch * 43 + case_seed) % 13);
    const double epoch_yz = 0.00017 * (1 + (field_epoch * 47 + case_seed) % 11);
    const double epoch_xz = 0.00013 * (1 + (field_epoch * 53 + case_seed) % 7);
    const double epoch_xyz = 0.00011 * (1 + (field_epoch * 61 + case_seed) % 5);
    const double epoch_radial = 0.00009 * (1 + (field_epoch * 67 + case_seed) % 29);
          ...
          for (int k = 0; k < cg->shape[2]; k++)
            for (int j = 0; j < cg->shape[1]; j++)
              for (int i = 0; i < cg->shape[0]; i++)
              {
                ...
                double scaled_z = z / scale;
                double scaled_xy = scaled_x * scaled_y;
                double scaled_yz = scaled_y * scaled_z;
                double scaled_xz = scaled_x * scaled_z;
                double scaled_xyz = scaled_xy * scaled_z;
                double scaled_shape = scaled_x * scaled_x - scaled_y * scaled_y +
                                      0.5 * scaled_z * scaled_z;
                int grid_x = static_cast<int>(floor(
                  (x - patch->bbox[0]) / patch->getdX(0) + 0.5));
                int grid_y = static_cast<int>(floor(
                  (y - patch->bbox[1]) / patch->getdX(1) + 0.5));
                int grid_z = static_cast<int>(floor(
                  (z - patch->bbox[2]) / patch->getdX(2) + 0.5));
                double real_noise = benchmark_field_noise(
                  field_epoch, case_seed, grid_x, grid_y, grid_z,
                  0x7265616c2d707369ULL);
                double imag_noise = benchmark_field_noise(
                  field_epoch, case_seed, grid_x, grid_y, grid_z,
                  0x696d61672d707369ULL);

                cg->fgfs[Rpsi4->sgfn][n] = real_scale *
                                            (1.0 + (linear_x + epoch_x) * scaled_x +
                                             (linear_y + epoch_y) * scaled_y +
                                             (linear_z + epoch_z) * scaled_z +
                                             (mixed_xy + epoch_xy) * scaled_xy +
                                             (mixed_yz + epoch_yz) * scaled_yz +
                                             (mixed_xz + epoch_xz) * scaled_xz +
                                             epoch_xyz * scaled_xyz +
                                             epoch_radial * scaled_shape +
                                             0.002 * real_noise);
                cg->fgfs[Ipsi4->sgfn][n] = imag_scale *
                                            (scaled_z + (linear_x - epoch_y) * scaled_x +
                                             (linear_y + epoch_x) * scaled_y +
                                             (mixed_xy - epoch_xz) * scaled_xy +
                                             (mixed_yz + epoch_xy) * scaled_yz +
                                             (mixed_xz - epoch_yz) * scaled_xz +
                                             epoch_xyz * scaled_xyz +
                                             epoch_radial * scaled_z * scaled_z +
                                             0.04 * imag_noise);
              }
        }
        if (BP == Pp->data->ble)
    ...
    Waveshell->begin_wave_benchmark(a_lev, GH, Rpsi4, Ipsi4, field_epoch);
    double Rex = maxrex;
    for (int detector = 0; detector < decn; detector++)
    {
      Waveshell->surf_Wave(Rex, a_lev, GH, Rpsi4, Ipsi4, 2, maxl, NN, RP, IP, ErrorMonitor);
      if (include_mass)
        Waveshell->surf_MassPAng(Rex, a_lev, GH, phi0, trK0,
                                 gxx0, gxy0, gxz0, gyy0, gyz0, gzz0,
                                 Axx0, Axy0, Axz0, Ayy0, Ayz0, Azz0,
                                 Gmx0, Gmy0, Gmz0, Sfx_rhs, Sfy_rhs, Sfz_rhs,
                                 Rout, ErrorMonitor);
      if (benchmark_iteration >= 0)
      {
        double result_weight = 1.0 + 0.0001 * (benchmark_iteration + 1) +
                               0.00001 * (detector + 1);
        for (int i = 0; i < NN; i++)
        {
          wave_r += result_weight * (i + 1) * fabs(RP[i]);
          wave_i += result_weight * (NN + i + 1) * fabs(IP[i]);
          spectral_energy += result_weight *
                             ((i + 1) * RP[i] * RP[i] +
                              (NN + i + 1) * IP[i] * IP[i]);
        }
        for (int i = 0; i < 7; i++)
          Rout_accum[i] += result_weight * Rout[i];
      }
      Rex -= drex;
    }
    Waveshell->end_wave_benchmark();
    iteration_time = MPI_Wtime() - iteration_time;
    if (benchmark_iteration >= 0)
      local_time += iteration_time;
  }
  ...
  if (myrank == 0)
  {
    double mass_checksum = 0;
    for (int i = 0; i < 7; i++)
      mass_checksum += (2 * NN + i + 1) * Rout_accum[i];
    double checksum = wave_r + wave_i + mass_checksum + spectral_energy;
    ...
    if (result_file[0] != '\0')
    {
      ...
      if (result == NULL)
      {
        cerr << "Cannot open benchmark result file " << result_file << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
          fprintf(result, "%.17g %.17g %.17g %.17g %.17g\n",
              checksum, wave_r, wave_i, Rout_accum[0], spectral_energy);
      fclose(result);
    }

    cout << setprecision(17);
    cout << "BLACKHOLE_RESULT checksum=" << checksum
         << " wave_r=" << wave_r
         << " wave_i=" << wave_i
         << " mass=" << Rout_accum[0]
         << " energy=" << spectral_energy << endl;
    cout << "BLACKHOLE_TIME max=" << max_time
         << " average=" << sum_time / nprocs
         << " repeats=" << repeats
         << " detectors=" << decn
         << " mass=" << include_mass << endl;
  }
